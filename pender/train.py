...
def train(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        initial_batch_step = (global_epoch_fraction
                - int(global_epoch_fraction)) * int(data_loader.total_batch_count)
        ...
        global_step = epoch_range[0] * int(data_loader.total_batch_count) + initial_batch_step
        try:
            for e in range(*epoch_range):
                ...
                batch_range = (int(initial_batch_step), int(data_loader.total_batch_count))
                ...
                for b in range(*batch_range):
                    ...
                    if (e * batch_range[1] + b + 1) % args.save_every == 0 \
                        ...
                        save_model(sess, saver, model, args.save_dir, global_step,
                                int(data_loader.total_batch_count), global_seconds_elapsed)
            ...
            writer.flush()
            global_step = e * int(data_loader.total_batch_count) + b
            save_model(sess, saver, model, args.save_dir, global_step,
                    int(data_loader.total_batch_count), global_seconds_elapsed)
