...
class TextLoader():
    ...
    def _save_vocab(self, vocab_counter, vocab_file):
        ...
        with open('../../project/reddit/chars_vocab.pkl', 'rb') as f:
    ...
    def _preprocess(self, input_file, tensor_file):
        ...
        ts = []
        for d in data:
            ts.append(self.vocab.get(d, 0))
        self.tensor = np.array(ts[:-5])
        #self.tensor = np.array(map(self.vocab.get, data))
        print('####### tensor %s, data %s' % (self.tensor.shape, len(data)))
        if not self.tensor.shape:
    ...
    def _load_preprocessed(self, tensor_index):
        ...
        print('Tensor shape %s, num batches %s, batch size %s, seq len %s' % (
                self.tensor.shape, self.num_batches, self.batch_size, self.seq_length))
